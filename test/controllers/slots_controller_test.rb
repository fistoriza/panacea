...
class SlotsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create slot" do
    assert_difference('Slot.count') do
      post slots_url, params: { slot: { starting_time: @slot.starting_time, status: @slot.status } }
    end
  ...
  test "should update slot" do
    patch slot_url(@slot), params: { slot: { starting_time: @slot.starting_time, status: @slot.status } }
